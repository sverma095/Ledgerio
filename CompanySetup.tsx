...
export default function CompanySetup({ db, onUpdateCompany, onUpdateRole, onResetDB, currentUserEmail, initialSection, hideMenu }: CompanySetupProps) {
  ...
  const [bankAccountHolder, setBankAccountHolder] = useState(db.company.bankAccountHolder || "");
  const [bankBranch, setBankBranch] = useState(db.company.bankBranch || "");
  const [paymentInstructions, setPaymentInstructions] = useState(db.company.paymentInstructions || "");
  const [tagline, setTagline] = useState(db.company.tagline || "");
                ...
                <form onSubmit={handleProfileSave} className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5 col-span-2">
                      <label className="text-xs text-slate-500 font-bold">Tagline (shown on invoices, optional)</label>
                      <input 
                        type="text"
                        value={tagline}
                        onChange={(e) => setTagline(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 focus:bg-white rounded-lg px-3.5 py-2 text-slate-800 text-xs focus:border-[#006EE5] outline-none transition"
                        placeholder="e.g. Trusted accounting partner since 2015"
                      />
                    </div>
                  </div>
                  ...
                  <div className="border-t pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs text-slate-500 font-bold">Account Holder Name</label>
                        <input type="text" value={bankAccountHolder} onChange={(e) => setBankAccountHolder(e.target.value)} placeholder="e.g. Verma Consultancy Services"
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2 text-slate-800 text-xs focus:border-[#006EE5] outline-none" />
                      </div>
                      <div className="space-y-1.5">
                      ...
                      <div className="space-y-1.5">
                        <label className="text-xs text-slate-500 font-bold">Branch</label>
                        <input type="text" value={bankBranch} onChange={(e) => setBankBranch(e.target.value)} placeholder="e.g. MG Road, Lucknow"
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2 text-slate-800 text-xs focus:border-[#006EE5] outline-none" />
                      </div>
                      <div className="space-y-1.5">
                      ...
                      <div className="space-y-1.5 col-span-2">
                        <label className="text-xs text-slate-500 font-bold">Payment Instructions (shown on invoice)</label>
                        <input type="text" value={paymentInstructions} onChange={(e) => setPaymentInstructions(e.target.value)} placeholder="e.g. Please mention invoice number in the payment reference"
                          className="w-full bg-slate-50 border border-slate-200 rounded-lg px-3.5 py-2 text-slate-800 text-xs focus:border-[#006EE5] outline-none" />
                      </div>
                    </div>
                  </div>
